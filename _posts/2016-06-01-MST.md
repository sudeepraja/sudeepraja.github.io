...
### Approach 2
On seeing a $$Change(e,w)$$, modify the weight and also find the modified MST. Assume $$w_{old}$$ is the weight of edge $$e$$ before modification. Let this be the original graph. The black edges are part of the MST and the grey ones are not.
<p align="center">
...
Approach 2: Since case 4 is the costliest. - $$\alpha O(\mid E \mid) + \beta O(\mid V \mid)$$

Approach 2 is efficient when:

$$\alpha O(\mid E \mid) + \beta O(\mid V \mid) < \alpha O(1) + \beta( O( \mid E \mid \log \mid V \mid) + O(\mid V \mid))\\
\alpha(\mid E \mid-1) < \beta (O( \mid E \mid \log \mid V \mid)) \\
