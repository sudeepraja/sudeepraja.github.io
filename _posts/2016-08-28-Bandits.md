...
A Multi Armed Bandit consists of $$K$$ arms, $$K\ge2$$ numbered from $$1$$ to $$K$$. Each arm $$i$$ is associated with an unknown probability distribution $$P_i$$ with whose mean is $$\mu_i$$. Pulling the $$i$$th arm produces a reward $$r$$ which is sampled from $$P_i$$. There is an agent which has a budget of $$T$$ arm pulls. The task of the agent is to maximize the accumulated reward after $$T$$ arm pulls.
...
![enter image description here](https://raw.githubusercontent.com/sudeepraja/sudeepraja.github.io/master/images/bandit_alg.PNG =100x)
