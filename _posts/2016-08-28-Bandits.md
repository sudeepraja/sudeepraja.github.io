...
A Multi Armed Bandit consists of $$K$$ arms, $$K\ge2$$ numbered from $$1$$ to $$K$$. Each arm $$a$$ is associated with an unknown probability distribution $$P_a$$ whose mean is $$\mu_a$$. Pulling the $$a$$th arm produces a reward $$r$$ which is sampled from $$P_a$$. There is an agent which has a budget of $$T$$ arm pulls. The agent has to pull these arms in some sequence so as to maximize the accumulated reward after $$T$$ arm pulls. The reward for pulling arm $$a_i$$ at the $$t$$'th step be $$r_{a_i,t}$$ which is sampled from $$P_{a_i}$$. The agent's task is to maximise $$\sum_{t=1}^T r_{a_i,t}$$ is maximized.

$$Maximize \quad \sum_{t=1}^T r_{a_i,t} \quad \mbox{where } r_{x_i,t} \sim P_{x_i} \mbox{ and }x_i \in \{a_1,a_2,..,a_K\}$$  
