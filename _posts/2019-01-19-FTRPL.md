...
2. For $$t=1...T$$
	1. Let $$p_t$$ be a distribution on $$\{0,1\}^n$$ such that $$\mathbb{E}_{X \sim p_t}[X]=x_t$$. Player plays $$X_t \sim p_t$$.
	2. See the loss vector $$l_{t} \in [-1,1]^n$$ and incur loss $$l_t^\top X_t$$.
	3. Update  $$x_{t+1} = \arg \min_{x \in [0,1]^n} \left[\eta \sum_{\tau=1}^t l_\tau^\top x + R(x)\right]$$
...
2. For $$t=1...T$$
	1. Player plays $$X_t$$.
	2. See the loss vector $$l_{t} \in [-1,1]^n$$ and incur loss $$l_t^\top X_t$$.
	3. Update  $$X_{t+1} = \arg \min_{x \in \{0,1\}^n} \left[\eta \sum_{\tau=1}^t l_\tau^\top x + v^\top x\right]$$ where $$v=[v_1, v_2, \dots, v_n]$$ are drawn iid from the Logistic distribution.

First, note that the minimization step $$\arg \min_{x \in \{0,1\}^n} \left[\eta \sum_{\tau=1}^t l_\tau^\top x + v^\top x\right]$$ can be solved easily. As it is a minimization problem, $$X_i=1$$ if $$\eta \sum_{\tau=1}^t l_{i,\tau} + v_i \leq 0$$ and $$X_i=0$$ otherwise. We do not need to do Bernoulli sampling as, we get a vertex of the hypercube directly from the minimization.

Next, as $$v$$ is a random variable, the output of the minimization step $$X_{t+1}$$ will be a random variable. Moreover, using the above observation and the fact that $v$ is a vector of iid random variables, we can conclude that $$X_{i,t+1}$$ are also independent. We have that:
...
To obtain the last equality, we use the fact that $$v_i$$ is drawn from the Logistic distribution. We can see that is equal to the probability of picking of picking $$X_{i,t+1}=1$$ in the PolyExp algorithm as $$X_{i,t+1} \sim Bernoulli(x_{i,t+1})$$ where $$x_{i,t+1} = (1+\exp(\eta \sum_{\tau=1}^t l_{i,\tau}))^{-1}$$

In conclusion, we have the following equivalences:
